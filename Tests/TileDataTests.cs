...
public class TileDataTests
{
    ...
    public void GetSpriteTileData()
    {
        ...
        Assert.AreEqual(0b_01, tile.GetColorIndex(0, 0));
        Assert.AreEqual(0b_11, tile.GetColorIndex(0, 1));
        Assert.AreEqual(0b_10, tile.GetColorIndex(0, 2));

        vram[16] = 6;
        vram[17] = 3;
        tile = ImprovedTileData.GetSpriteTileData(vram, 1);

        Assert.AreEqual(0b_10, tile.GetColorIndex(0, 0));
        Assert.AreEqual(0b_11, tile.GetColorIndex(0, 1));
        Assert.AreEqual(0b_01, tile.GetColorIndex(0, 2));

        vram[0x1000 - 16] = 1;
        vram[0x1000 - 15] = 3;
        tile = ImprovedTileData.GetSpriteTileData(vram, 0xff);

        Assert.AreEqual(0b_11, tile.GetColorIndex(0, 0));
        Assert.AreEqual(0b_10, tile.GetColorIndex(0, 1));
        Assert.AreEqual(0b_00, tile.GetColorIndex(0, 2));
    }
    ...
    public void GetTileData()
    {
        ...
        Assert.AreEqual(0b_11, tile.GetColorIndex(0, 0));
        Assert.AreEqual(0b_01, tile.GetColorIndex(0, 1));
        Assert.AreEqual(0b_00, tile.GetColorIndex(0, 2));
